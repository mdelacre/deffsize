...
server = function(input, output) {
  ...
  observeEvent(input$designBut, {
    ...
    v$text = paste((1-input$alpha)*100,"% Confidence interval:")
    ...
    if (input$corr == 1 && input$var == 'No') {
      ...
      v$tab = format.data.frame(df, digits = 3)
      
    } else
      if (input$corr == 0 && input$var == 'No') {
        ...
        if (Glass_d &
            Shieh_d & Hedges_dprime) {
          df <- rbind(df5, df6, df7)
    ...
    if (input$corr == 1 && input$var == 'No') {
      ...
      DF1 <- N1 - 1
      #DF <- input$N1-1
      LAMBDA1 <- (M1 - M2) / (SD1 * sqrt(1 / N1 + SD2 ^ 2 / (N2 * SD1 ^
                                                               2)))
      ...
      DF2 <-
        ((SD1 ^ 2 / N1 + SD2 ^ 2 / N2) ^ 2) / ((SD1 ^ 2 / N1) ^ 2 / (N1 - 1) +
                                                 (SD2 ^ 2 / N2) ^ 2 / (N2 - 1))
      ...
      DF3 <-
        ((N1 - 1) * (N2 - 1) * (SD1 ^ 2 + SD2 ^ 2) ^ 2) / ((N2 - 1) * SD1 ^ 4 +
                                                             (N1 - 1) * SD2 ^ 4)
      ...
      if (input$corr == 0 && input$var == 'No') {
        ...
        if (input$corr == 1 && input$var == 'Yes') {
          ...
          if (Hedges_g) {
            ...
            LAMBDA <-
              (M1 - M2) / (sqrt(((N1 - 1) * SD1 ^ 2 + (N2 - 1) * SD2 ^ 2) / (N1 + N2 -
                                                                               2)) * sqrt(1 / N1 + 1 / N2))
